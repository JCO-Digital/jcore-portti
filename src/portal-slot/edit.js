...
import { PanelBody, SelectControl, ExternalLink, Placeholder } from '@wordpress/components';
...
export default function Edit({ attributes, setAttributes }) {
	...
	const { campaignPosts } = useSelect(
		(select) => {
			...
			return {
				campaignPosts: getEntityRecords('postType', 'jcore-portal-content', query) || [],
			};
		},
		[slotId, terms]
	);
		...
		<InspectorControls>
			<PanelBody title={__('Settings', 'jcore-portti')}>
				<SelectControl
					...
					help={__('Choose the slot location for this block.', 'jcore-portti')}
				/>
				{slotId && (
					<SelectControl
						...
						onChange={(value) => setAttributes({ previewPostId: parseInt(value, 10) })}
						help={__(
		...
		<div {...useBlockProps()}>
			...
			<div className="jcore-portal-slot-fallback-editor">
				...
				<p style={{ fontSize: '11px', opacity: 0.7, marginBottom: '10px' }}>
					{__('Fallback Content (Shown if no active campaign matches):', 'jcore-portti')}
				</p>
				<InnerBlocks />
			</div>
		</div>
